...
class uploadImage extends Component {
  ...
  render() {
    const { previewVisible, previewImage, loading, headers } = this.state;
    const { user, uploadAvatarImage, type, productImage } = this.props;
    const { image } = uploadAvatarImage;
    const uploadButton = user.user.avatarUrl ? (
      <img src={user.user.avatarUrl} alt="avatar" style={{ width: "100%" }} />
    ) : (
      <div>
